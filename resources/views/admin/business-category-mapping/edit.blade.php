...
<div class="grid grid-cols-1 lg:gap-16 md:gap-10">
    <div class="bg-white shadow-[0px_6px_16px_rgba(0,0,0,0.05)] rounded-xl">
        ...
        <div class="flex justify-between items-center p-5 border-b">
            <h2 class="text-lg font-semibold">Edit Business Category Mapping</h2>
        ...
        {{-- Form --}}
        <form action="{{ route('business-category-mapping.update', $mapping->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select name="business_category_id" id="business_category"
                        ...
                        @foreach ($businessCategories as $category)
                            <option value="{{ $category->id }}"
                                {{ old('business_category_id', $mapping->business_category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="category_id"
                        ...
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}"
                                {{ old('category_id', $mapping->category_id) == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="status"
                        class="w-full border rounded-lg px-3 py-2">
                        <option value="1" {{ old('status', $mapping->status) == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ old('status', $mapping->status) == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
